...
public class Cart extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("Cart") != null && request.getParameter("Cart").equals("CheckOut")) {
			...
			utility.storeProduct(name, type, maker, access, extraCost);
			displayCart(request, response);
		}
	}

	private double calculatePrice(OrderItem oi) {
		...
		if (utility.CartCount() > 0) {
			...
			for (OrderItem oi : utility.getCustomerOrders()) {
				...
				pw.print("<td>" + i + ".</td><td>" + oi.getName() + "</td><td>: "
						+ calculatePrice(oi)
						+ "</td><td>: <input type='text' size='4' name='orderAmount" + oi.getId() + "' value='"
						+ oi.getAmount() + "'></td>");
				pw.print("<td><input type='submit' name='Update' value='Update " + oi.getId()
						+ "' class='btnbuy'></td>");
				pw.print("<td><input type='submit' name='Delete' value='Delete " + oi.getId()
						+ "' class='btnbuy'></td>");
				pw.print("<input type='hidden' name='orderItemId' value='" + oi.getId() + "'>");
				pw.print("<input type='hidden' name='orderName' value='" + oi.getName() + "'>");
				pw.print("<input type='hidden' name='orderPrice' value='"
						+ calculatePrice(oi) + "'>");
				pw.print("</tr>");
				total = total + (calculatePrice(oi) * oi.getAmount());
				i++;
			}
			pw.print("<input type='hidden' name='orderTotal' value='" + total + "'>");
			pw.print("<tr><th></th><th>Total</th><th>" + total + "</th>");
			pw.print(
					"<tr><td></td><td></td><td><input type='submit' name='Cart' value='CheckOut' class='btnbuy'></td>");
			pw.print("</table>");
		} else {
